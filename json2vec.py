...
class JSONTreeLSTM(JSONNN):
    def __init__(self, mem_dim=128,
        ...
        self.string_encoder_w2v = spacy.load('en_core_web_md')
        self.string_encoder_out = keydefaultdict(self._newstringout)
    ...
    def embedArray___(self, child_states, path):
    ...
    def _new_embedder(self, key):
        l1 = torch.nn.Linear(self.mem_dim*2, self.mem_dim)
    ...
    def embedArray(self, child_states, path):
        ...
        for c in child_states:
            c_t = self.embedder[canon_path](torch.cat(c, dim=1))
            c_ts.append(c_t)
        ...
        return self.out[canon_path](torch.max(torch.cat(c_ts),
            dim=0, keepdim=True)[0]), self._initC()
    ...
    def embedString___(self, string, path):
    ...
    def _newstringout(self, key):
        layer = nn.Linear(300, self.mem_dim)
        self.add_module(str(key)+"_stringout", layer)
        return layer

    def embedString(self, string, path):
        canon_path = self._canonical(path) if not self.tie_primitives else "___default___"
        enc_str = self.string_encoder_w2v(string)
        encoded_input = torch.Tensor([word.vector for word in enc_str])\
                            .view(-1, 1, 300)
        return self._initC(), self.string_encoder_out[canon_path](encoded_input.mean(dim=0))
